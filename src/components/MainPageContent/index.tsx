    ...
    return (
        <div className={styles.MainContent} >
            <Space direction={'vertical'} size='large' className={styles.MainSpace}>
                <Card >
                    <h6 className='color_blue'>{`С CleverFit ты сможешь:
                ...
                <Space direction={'vertical'} size='middle'>
                    ...
                    <Row gutter={[16, 8]} className={styles.Cards}>
                        {CARDS_INFO.map(({ title, text, icon, key }) =>
                            <Col key={key} sm={8} xs={24}>
                                <Card bordered={false} className={styles.Card} bodyStyle={{ padding: '12px 30px' }} key={key}
                                    actions={
